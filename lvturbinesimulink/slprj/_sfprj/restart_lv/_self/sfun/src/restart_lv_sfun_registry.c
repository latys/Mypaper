...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_restart_lv_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_restart_lv_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_restart_lv_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_restart_lv_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_restart_lv_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "restart_lv") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_restart_lv_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_restart_lv_method_dispatcher(SimStruct *S, unsigned int
...
{
  if (!strcmp(machineName,"restart_lv")) {
    return(sf_restart_lv_method_dispatcher(simstructPtr,chartFileNumber,method,
            data));
  }
...
extern void restart_lv_terminator(void);
...
extern void restart_lv_initializer(void);
extern void restart_lv_register_exported_symbols(SimStruct* S);
extern void restart_lv_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  restart_lv_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      restart_lv_debug_initialize();
    }

    restart_lv_initializer();
  }
