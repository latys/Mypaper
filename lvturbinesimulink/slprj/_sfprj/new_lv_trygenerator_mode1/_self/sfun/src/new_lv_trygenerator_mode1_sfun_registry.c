...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_new_lv_trygenerator_mode1_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_new_lv_trygenerator_mode1_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_new_lv_trygenerator_mode1_autoinheritance_info( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_new_lv_trygenerator_mode1_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_new_lv_trygenerator_mode1_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "new_lv_trygenerator_mode1") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_new_lv_trygenerator_mode1_get_eml_resolved_functions_info(nlhs,
      plhs,2,newRhs);
  }
...
extern unsigned int sf_new_lv_trygenerator_mode1_method_dispatcher(SimStruct *S,
...
{
  if (!strcmp(machineName,"new_lv_trygenerator_mode1")) {
    return(sf_new_lv_trygenerator_mode1_method_dispatcher(simstructPtr,
...
extern void new_lv_trygenerator_mode1_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    new_lv_trygenerator_mode1_terminator();
    sf_debug_terminate();
  }
...
extern void new_lv_trygenerator_mode1_initializer(void);
extern void new_lv_trygenerator_mode1_register_exported_symbols(SimStruct* S);
extern void new_lv_trygenerator_mode1_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  new_lv_trygenerator_mode1_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      new_lv_trygenerator_mode1_debug_initialize();
    }

    new_lv_trygenerator_mode1_initializer();
  }
